...
for id, folder in enumerate(files):
    if os.path.isdir(directory + folder):
        ...
        postreward_index = np.concatenate([list(range(i+int(reward_duration*rec.sf), i+int(postreward_duration * rec.sf))) for i in rec.reward_time])
        ...
        for i in range(n_regressors):
            neuron_labels[coef[:, i] > 2*coef_se[:, i], i] = 1  # if the coefs are larger than 2*SE we set the sign to 1
