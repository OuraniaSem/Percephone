...
def calculate_pvalue(statistic, statistic_bootstrap):
    """Calculate p-values for a set of statistics, based on a null distribution estimated with a
    set of bootstrapped statistics.
...
def stationary_bootstrap(signal, average_block_size=100):
    """resample a time series using stationary bootstrap, it samples with replacement random
    blocks of sizes following a geometric distribution.
...
def calculate_covariance_bootstrap(signal, dff, n_resamples=50, average_block_size=240):
    """Calculate covariance values between resampled signal using stationary bootstrap and
    rescaled fluorescence for all neurons.
    ...
    for i in tqdm(range(n_resamples)):
        ...
        covariance_bootstrap[i * n_neurons:(i + 1) * n_neurons] = (
            calculate_covariance(signal_resampled, dff))  # calculate covariances with neural traces
...
def find_significant_neurons(statistic, statistic_bootstrap, max_neurons, alpha_bh=0.05, ):
    ...
    indices_thr = indices_sorted[
                  :n_significant_neurons]  # only take the ones for which the statistic is significantly larger than 0
